...
try:
    ...
    _logger.warning("OpenEXR/Imath libraries not found. EXR features will be unavailable in GUI.")
# --- Imports End ---
...
class DepthCrafterGUI:
    ...
    def __init__(self, root):
        ...
        self.current_input_mode = (
            "batch_folder"  # "batch_folder", "single_video_file", "single_image_file", "image_sequence_folder"
        )
        ...
        self.use_sageattention_var = tk.BooleanVar(value=True)
        ...
        self.all_tk_vars = {
            ...
            "use_sageattention_var": self.use_sageattention_var,
        }
    ...
    def _apply_all_settings(self, settings_data: dict):
        for key, value_from_json in settings_data.items():
            if key == "target_fps":  # Specific debug
                _logger.debug(
                    f"_apply_all_settings: Loading target_fps from JSON. Value: {value_from_json}, Type: {type(value_from_json)}"
                )
            if key in self.all_tk_vars:
                try:
                    ...
                    if key == "target_fps":
                        ...
                        _logger.debug(
                            f"_apply_all_settings: target_fps in DoubleVar after set: {val_in_doublevar}, Type: {type(val_in_doublevar)}"
                        )
                except tk.TclError:
                    _logger.warning(
                        f"Warning: Could not set value for setting '{key}' to '{value_from_json}'. Skipping."
                    )
            else:
    ...
    def _apply_theme(self, is_startup: bool = False):
        ...
        if hasattr(self, "menubar"):
            ...
            self.menubar.config(
                bg=root_bg_color, fg=root_fg_color, activebackground=menu_active_bg, activeforeground=menu_active_fg
            )
            if hasattr(self, "file_menu"):
                self.file_menu.config(bg=root_bg_color, fg=root_fg_color)
            if hasattr(self, "help_menu"):
                self.help_menu.config(bg=root_bg_color, fg=root_fg_color)
        ...
        if not self.keep_intermediate_npz_var.get():
            ...
            if min_frames > 0:
                ...
                if orig_frames < min_frames:
                    _logger.info(
                        f"  Video frames ({orig_frames}) < threshold ({min_frames}). Deleting segment folder for {original_basename} despite 'Keep NPZ'."
                    )
                    del_folder = True
                else:
                    _logger.info(
                        f"  Video frames ({orig_frames}) >= threshold ({min_frames}). Segment folder for {original_basename} will be kept."
                    )
            else:
                _logger.debug(
                    f"Keeping intermediate NPZ files for {original_basename} (Keep NPZ checked, no positive frame threshold)."
                )
        if del_folder:
    ...
    def _create_menubar(self):
        ...
        self.file_menu.add_command(
            label="Restore Finished Input Files...", command=lambda: self._restore_input_files(folder_type="finished")
        )
        self.file_menu.add_command(
            label="Restore Failed Input Files...", command=lambda: self._restore_input_files(folder_type="failed")
        )
        self.file_menu.add_separator()
        self.file_menu.add_checkbutton(
            label="Use Local Models Only", variable=self.use_local_models_only_var, onvalue=True, offvalue=False
        )
        self.file_menu.add_checkbutton(
            label="Use SageAttention (Faster)", variable=self.use_sageattention_var, onvalue=True, offvalue=False
        )
        if THEMEDTK_AVAILABLE:
        ...
        self.help_menu.add_checkbutton(
            label="Enable Debug Logging", variable=self.debug_logging_enabled, command=self._toggle_debug_logging
        )
    ...
    def _determine_input_mode_from_path(self, path_str: str) -> Tuple[str, bool]:
        ...
        if not path_str or not os.path.exists(path_str):
            _logger.warning(
                f"GUI Input: Path '{path_str}' is invalid or does not exist. Cannot determine input mode accurately."
            )
        ...
        if os.path.isfile(path_str):
            ...
            if is_video:
                ...
                _logger.warning(
                    f"GUI Input: Typed path '{path_str}' is a file of unknown type. Treating as non-single source (batch fallback)."
                )
                mode = "batch_folder"
                is_single_source = False
        ...
        _logger.debug(
            f"GUI Input: Determined mode for path '{path_str}' as '{mode}', is_single_source: {is_single_source}."
        )
    ...
    def _execute_re_merge(self, remerge_args_dict):
        ...
        try:
            if merge_depth_segments:
                ...
                _logger.warning(
                    "Segment merging for N/A for re-merge action skipped: merge_depth_segments module not available."
                )
        except Exception as e:
            ...
            _logger.info(
                f"--- Re-Merge for: {os.path.basename(remerge_args_dict['master_meta_path'])} finished in {duration}. ---"
            )
    ...
    def _execute_generate_segment_visuals(self, gen_visual_args_dict):
        ...
        jobs = [
            j
            for j in meta_data.get("jobs_info", [])
            if j.get("status") == "success" and j.get("output_segment_filename")
        ]
        if not jobs:
            _logger.warning(
                f"No successful segments with output filenames found in {os.path.basename(master_path)} for visual generation."
            )
            return
        ...
        for i, job_meta in enumerate(jobs):
            ...
            _logger.debug(
                f"  Visual Gen - Processing segment {seg_id + 1 if seg_id is not None else '?'}/{len(jobs)}: {npz_name} for {vis_fmt}"
            )
            ...
            try:
                ...
                norm_frames = (
                    (raw_frames - raw_frames.min()) / (raw_frames.max() - raw_frames.min())
                    if raw_frames.max() != raw_frames.min()
                    else np.zeros_like(raw_frames)
                )
                ...
                fps = float(
                    job_meta.get(
                        "processed_at_fps", meta_data.get("original_video_details", {}).get("original_fps", 30.0)
                    )
                )
                if fps <= 0:
                    fps = 30.0
                ...
                if vis_fmt == "mp4" or vis_fmt == "main10_mp4":
                    ...
                    save_path, save_err = save_depth_visual_as_png_sequence_util(
                        norm_frames, seg_folder_path, base_name_no_ext
                    )
                elif vis_fmt == "exr_sequence":
                    if OPENEXR_AVAILABLE_GUI:
                        save_path, save_err = save_depth_visual_as_exr_sequence_util(
                            norm_frames, seg_folder_path, base_name_no_ext
                        )
                    else:
                        save_err = "OpenEXR module not available in GUI environment."
                elif vis_fmt == "exr":
                    if OPENEXR_AVAILABLE_GUI:
                        first_frame = norm_frames[0] if len(norm_frames) > 0 else None
                        if first_frame is None:
                            save_err = "No frame data for single EXR."
                        else:
                            save_path, save_err = save_depth_visual_as_single_exr_util(
                                first_frame, seg_folder_path, base_name_no_ext
                            )
                    else:
                        save_err = "OpenEXR module not available in GUI environment."
                ...
                if save_path:
                    ...
                    if seg_id is not None:
                        updated_visual_paths[seg_id] = {"path": os.path.abspath(save_path), "format": vis_fmt}
                if save_err:
                    _logger.error(
                        f"    Visual Gen - ERROR saving visual for {npz_name}: {save_err}, format requested: {vis_fmt}"
                    )
            except Exception as e:
        ...
        if updated_visual_paths:
            ...
            if meta_content_update:
                ...
                if updated_count > 0:
                    ...
                    _logger.info("Visual Gen - No segments in master metadata needed visual path updates.")

        duration = format_duration(time.perf_counter() - start_time)
        _logger.info(
            f"--- Segment Visual Generation for: {os.path.basename(master_path)} (Format: {vis_fmt}) finished in {duration}. ---"
        )
    ...
    def _finalize_video_processing(self, current_video_path, original_basename, master_meta_for_this_vid):
        ...
        _logger.debug(
            f"Finished processing for {original_basename}. Overall Status: {master_meta_for_this_vid['overall_status']}."
        )

        main_output_dir, segment_subfolder_path, was_segments = self._determine_video_paths_and_processing_mode(
            original_basename, master_meta_for_this_vid
        )
        ...
        try:
            master_meta_filepath, meta_saved = self._save_master_metadata_and_cleanup_segment_json(
                master_meta_for_this_vid, original_basename, main_output_dir, was_segments, segment_subfolder_path
            )

            if (
                was_segments
                and meta_saved
                and master_meta_for_this_vid["overall_status"] in ["all_success", "partial_success"]
            ):
                try:  # Nested try-except to catch errors specifically from merging
                    merge_success, final_merged_path = self._handle_segment_merging(
                        master_meta_filepath, original_basename, main_output_dir, master_meta_for_this_vid
                    )
                ...
                _logger.debug(
                    f"Skipping merge for {original_basename} (status: {master_meta_for_this_vid['overall_status']}, meta_saved: {meta_saved}). Segments remain in {segment_subfolder_path or 'N/A'}"
                )
                # No change to merge_success/final_merged_path as they were initialized to False/N/A

            if self.save_final_output_json_var.get():
                self._save_final_output_sidecar_json(
                    original_basename,
                    final_merged_path,
                    master_meta_filepath,
        ...
        if self.effective_move_original_on_completion:
            ...
            if final_status == "all_success":
                ...
                _logger.warning(
                    f"Move Original: Could not determine 'finished' or 'failed' status for '{original_basename}' (status: '{final_status}'). Original file will not be moved."
                )
    ...
    def _get_segments_to_resume_or_overwrite(
        self,
        vid_path,
        original_basename,
        segment_subfolder_path,
        all_potential_segments_from_define,
        ...
        if os.path.exists(master_meta_path):
            msg_dialog = (
                f"Master metadata found for '{original_basename}'. This video was previously processed/finalized.\n"
            ...
            if choice is True:
                _logger.info(
                    f"Attempting to re-process failed segments for {original_basename} based on existing master metadata."
                )
                master_data = load_json_file(master_meta_path)
                if not master_data or "jobs_info" not in master_data:
                    _logger.warning(
                        f"Could not load master metadata or 'jobs_info' missing for {original_basename}. Defaulting to overwrite."
                    )
                    choice = False  # Fallthrough
                else:
                    failed_segment_jobs_to_run = []
                    successful_jobs_from_old_master = []
                    potential_segments_dict = {
                        seg_job["segment_id"]: seg_job for seg_job in all_potential_segments_from_define
                    }
                    ...
                    for job_in_meta in master_data.get("jobs_info", []):
                        ...
                        if job_in_meta.get("status") == "success":
                            ...
                            _logger.debug(
                                f"  Queueing segment ID {seg_id} (status: {job_in_meta.get('status', 'unknown')}) for {original_basename} for re-processing."
                            )
                        else:
                            _logger.warning(
                                f"  Warning: Segment (ID: {seg_id}, Status: {job_in_meta.get('status')}) from master_meta for {original_basename} not re-queueable. It will be ignored."
                            )

                    if not failed_segment_jobs_to_run:
                        _logger.info(
                            f"No re-processable failed segments found in master_meta for {original_basename}. All existing successful segments will be preserved if merging."
                        )
                    ...
                    try:
                        ...
                        _logger.debug(
                            f"Backed up existing file {os.path.basename(master_meta_path)} to: {os.path.basename(backup_master_meta_path)}"
                        )
                    except Exception as e:
                        _logger.warning(
                            f"  Warning: Could not back up existing master metadata: {e}. It might be overwritten."
                        )
            ...
            if choice is False:
                _logger.info(
                    f"User chose/defaulted to delete existing segment folder and start fresh for {original_basename}: {segment_subfolder_path}"
                )
                try:
                    if os.path.exists(segment_subfolder_path):
                        shutil.rmtree(segment_subfolder_path)
            ...
            msg_dialog_incomplete = (
                f"Incomplete segment data found for '{original_basename}' (no master metadata file).\n"
            ...
            choice_incomplete = messagebox.askyesnocancel(
                "Resume Incomplete Segments?", msg_dialog_incomplete, parent=self.root
            )
            ...
            if choice_incomplete is True:
                ...
                for potential_segment_job in all_potential_segments_from_define:
                    ...
                    if os.path.exists(npz_path) and os.path.exists(json_path):
                        ...
                        if segment_meta and segment_meta.get("status") == "success":
                            ...
                            status_msg = (
                                segment_meta.get("status", "unknown") if segment_meta else "JSON missing/corrupt"
                            )
                            _logger.info(
                                f"  Segment {seg_id + 1}/{total_segs} for {original_basename} found but not successful (status: {status_msg}). Will re-process."
                            )
                    else:
                        _logger.debug(
                            f"  Segment {seg_id + 1}/{total_segs} for {original_basename} (NPZ: {expected_npz_filename}) not found or JSON missing. Will process."
                        )
                ...
                if num_already_complete > 0:
                    _logger.info(
                        f"Found {num_already_complete} successfully completed segments for {original_basename} that will be skipped during processing."
                    )
                ...
                if not segments_to_run and num_already_complete == len(all_potential_segments_from_define):
                    _logger.warning(
                        f"  All segments for {original_basename} appear complete from individual files, but master_meta was missing. Consider re-merging. Skipping processing."
                    )
                    ...
                    _logger.warning(
                        f"  No segments to run for {original_basename}, but not all were found complete. Total defined: {len(all_potential_segments_from_define)}, Found complete: {num_already_complete}"
                    )
                ...
                _logger.info(
                    f"User chose to delete existing incomplete segment folder and start fresh for {original_basename}: {segment_subfolder_path}"
                )
                try:
                    if os.path.exists(segment_subfolder_path):
                        shutil.rmtree(segment_subfolder_path)
    ...
    def _handle_segment_merging(
        self, master_meta_filepath, original_basename, main_output_dir, master_meta
    ) -> Tuple[bool, str]:
        """
        ...
        if not merge_depth_segments:
            _logger.warning(
                f"Segment merging for {original_basename} skipped: merge_depth_segments module not available."
            )
        ...
        try:
            ...
            if primary_output_path is None:
                ...
                return False, "N/A (Merge module returned no path)"
            else:
    ...
    def _initialize_master_metadata_entry(
        self, original_basename, job_info_for_original_details, total_expected_jobs_for_this_video
    ):
        entry = {
        ...
        if self.process_as_segments_var.get():
            entry["global_processing_settings"]["segment_definition_output_window_frames"] = (
                job_info_for_original_details.get("gui_desired_output_window_frames", self.window_size.get())
            )
            entry["global_processing_settings"]["segment_definition_output_overlap_frames"] = (
                job_info_for_original_details.get("gui_desired_output_overlap_frames", self.overlap.get())
            )
        return entry
    ...
    def _load_help_content(self):
        if self._help_data is None:  # Only load once
            ...
            if raw_data:
                ...
                _logger.warning(
                    f"Warning: Could not load help content from {DepthCrafterGUI.HELP_CONTENT_FILENAME}. Tooltips will be limited/show 'not found'."
                )
        return self._help_data

    def _load_all_settings(self):
        filepath = filedialog.askopenfilename(
            title="Load Settings File", filetypes=self.SETTINGS_FILETYPES, initialdir=self.last_settings_dir
        )
        if not filepath:
        ...
        if settings_data:
            ...
            messagebox.showerror(
                "Load Error", f"Could not load settings from:\n{filepath}\nSee console log for details."
            )
    ...
    def _move_original_source(self, current_video_path: str, original_basename: str, target_subfolder: str):
        ...
        try:
            ...
            if os.path.isdir(path_from_gui_input_field):
                ...
                _logger.warning(
                    f"Move Original: The GUI input path '{path_from_gui_input_field}' is invalid for determining the target folder root. Using dirname of processed item as fallback."
                )
                ...
                if not os.path.isdir(actual_input_root_for_target_folder):
                    _logger.error(
                        f"Move Original: Cannot determine a valid root directory for target folder based on input path '{current_video_path}'."
                    )
                    _logger.error(
                        f"ERROR moving original '{original_basename}': Cannot determine valid root for target folder."
                    )
                    return
            ...
            if os.path.exists(current_video_path):
                if os.path.exists(dest_path):
                    ...
                    _logger.info(
                        f"Move Original: Destination already exists. Renaming '{dest_filename}' to '{new_dest_name}'."
                    )
    ...
    def _process_single_job(self, demo, job_info, master_meta_for_this_vid):
        ...
        try:
            ...
            if saved_data_filepath and returned_job_specific_metadata.get("status") == "success":
                ...
                log_msg_prefix_local = (
                    f"Segment {job_info.get('segment_id', -1) + 1}/{job_info.get('total_segments', 0)}"
                    if is_segment_job
                    else "Full video"
                )
                _logger.info(
                    f"  Job for {original_basename} ({log_msg_prefix_local}) status: {returned_job_specific_metadata.get('status', 'unknown_status')}"
                )
    ...
    def _restore_input_files(self, folder_type: str):  # Added folder_type argument
        ...
        if not messagebox.askyesno(
            f"Restore {display_folder_name} Input Files",
        ...
        if not os.path.isdir(source_input_path):
            messagebox.showerror(
                "Restore Error",
                f"Restore '{display_folder_name}' input files operation is only applicable when 'Input Folder/File' is set to a directory (batch mode).",
            )
            _logger.warning(
                f"Restore {display_folder_name} operation skipped: Input Folder/File is not a directory: {source_input_path}"
            )
                ...
                if os.path.isfile(src_path):
                    try:
                        if os.path.exists(dest_path):
                            ...
                            _logger.warning(
                                f"Input file '{filename}' already exists in '{source_input_path}'. Restoring as '{new_filename}'."
                            )
                        ...
                        errors_count += 1
                        _logger.error(
                            f"Error moving input file '{filename}' from '{finished_source_folder}': {e}", exc_info=True
                        )
        ...
        if restored_count > 0 or errors_count > 0:
            self.status_message_var.set(
                f"Restore {display_folder_name} complete: {restored_count} input files moved, {errors_count} errors."
            )
            messagebox.showinfo(
            ...
            messagebox.showinfo(
                "Restoration Complete", f"No input files found in the '{display_folder_name}' folder to restore."
            )

    def _save_all_settings_as(self):
        filepath = filedialog.asksaveasfilename(
            title="Save Settings As",
            filetypes=self.SETTINGS_FILETYPES,
            defaultextension=".json",
            initialdir=self.last_settings_dir,
        )
        if not filepath:
    ...
    def _save_final_output_sidecar_json(
        self, original_basename, final_merged_path, master_meta_filepath, master_meta, was_segments, merge_successful
    ):
        ...
        if was_segments:
            if merge_successful and final_merged_path and not final_merged_path.startswith("N/A"):
                ...
                json_content = {
                    "source_video_basename": original_basename,
                    "processing_mode": "segmented_then_merged",
                    "final_output_path": os.path.abspath(final_merged_path),
                    "final_output_format_selected": out_fmt_selected,
                    "master_metadata_path_source": os.path.abspath(master_meta_filepath)
                    if master_meta_filepath
                    else None,
                    ...
                    "merge_settings_summary": {
                        ...
                        "dithering": self.merge_dither_var.get(),
                        "dither_strength": self.merge_dither_strength_var.get(),
                        "gamma_correction": self.merge_gamma_correct_var.get(),
                        "gamma_value_if_applied": self.merge_gamma_value_var.get()
                        if self.merge_gamma_correct_var.get()
                        else 1.0,
                        "percentile_norm": self.merge_percentile_norm_var.get(),
                        "norm_low_perc": self.merge_norm_low_perc_var.get(),
                        "norm_high_perc": self.merge_norm_high_perc_var.get(),
                    },
                    "generation_timestamp_utc": time.strftime("%Y-%m-%dT%H:%M:%SZ", time.gmtime()),
                }
                if os.path.isdir(final_merged_path):
                    json_path = os.path.join(
                        os.path.dirname(final_merged_path.rstrip(os.sep)),
                        f"{os.path.basename(final_merged_path.rstrip(os.sep))}.json",
                    )
                elif os.path.isfile(final_merged_path):
                    json_path = get_sidecar_json_filename(final_merged_path)
                else:
                    _logger.warning(
                        f"    Cannot determine final JSON path for merged {original_basename} (output path: {final_merged_path})."
                    )
            else:
                _logger.info(
                    f"  Skipping final JSON for merged {original_basename} (merge not successful/path invalid)."
                )
        else:
            if master_meta and master_meta.get("jobs_info"):
                ...
                if relative_output_filename:
                    ...
                    if os.path.isdir(out_path):
                        json_path = os.path.join(
                            os.path.dirname(out_path.rstrip(os.sep)),
                            f"{os.path.basename(out_path.rstrip(os.sep))}.json",
                        )
                    elif os.path.isfile(out_path):
                        json_path = get_sidecar_json_filename(out_path)
                    else:
                        _logger.warning(
                            f"    Cannot determine final JSON path for full video {original_basename} (output path: {out_path})."
                        )
                else:
                    _logger.warning(
                        f"  Skipping final JSON for full video {original_basename} (output path/format missing from job_info)."
                    )
            else:
                _logger.warning(
                    f"  Skipping final JSON for full video {original_basename} (master_meta or job_info missing)."
                )
        ...
        if json_path and json_content:
            ...
            _logger.warning(
                f"  Final output JSON for {mode} '{original_basename}' not created (conditions not met, or save failed)."
            )

    def _save_master_metadata_and_cleanup_segment_json(
        self, master_meta_to_save, original_basename, main_output_dir, was_segments, segment_subfolder_path
    ):
        master_meta_filepath, meta_saved = None, False
        if was_segments:
            if not segment_subfolder_path:
                segment_subfolder_path = os.path.join(
                    main_output_dir, get_segment_output_folder_name(original_basename)
                )
        ...
        if should_save_master_meta_here:
            ...
            if was_segments and meta_saved and segment_subfolder_path:
                _logger.debug(
                    f"  Attempting to delete individual segment JSONs for {original_basename} (master created)."
                )
                deleted_count = 0
                    ...
                    if npz_file:
                        ...
                        if os.path.exists(json_to_del):
                            try:
                                ...
                                _logger.error(f"ERROR deleting individual segment JSON {json_to_del}: {e}")
                _logger.debug(f"    Deleted {deleted_count} individual segment JSONs.")
            elif was_segments and not meta_saved:
                _logger.warning(
                    f"  Skipping deletion of individual segment JSONs for {original_basename} (master_meta.json not saved)."
                )
        elif not was_segments:
            _logger.debug(
                f"Skipping save of '{os.path.basename(master_meta_filepath)}' by _save_master_metadata for full video mode for {original_basename}."
            )
            meta_saved = False
            ...
            if hasattr(widget, "configure"):
                try:
                    if isinstance(widget, ttk.Combobox):
                        widget.configure(state="disabled" if is_processing else "readonly")
                    else:
    ...
    def _show_help_for(self, help_key: str):
        ...
        help_text_widget = tk.Text(
            text_frame, wrap=tk.WORD, relief="flat", borderwidth=0, padx=5, pady=5, font=("Segoe UI", 9)
        )
    ...
    def _update_gui_info_on_job_finish(self, job_info_to_run, current_job_specific_metadata):
        ...
        if processed_h != "N/A" and processed_w != "N/A":
            ...
            final_display_res = (
                self.current_resolution_var.get() + " (Failed to confirm)"
            )  # Append if couldn't get actual
        ...
        if processed_frames_for_job != "N/A":
            ...
            final_display_frames_str = (
                self.current_frames_var.get() + " (Failed to confirm)"
            )  # Append if couldn't get actual
    ...
    def browse_single_input_file(self):
        filetypes = [
            ("All Supported", "*.mp4 *.avi *.mov *.mkv *.webm *.flv *.gif *.png *.jpg *.jpeg *.bmp *.tiff *.exr"),
        ...
        if os.path.isfile(initial_dir_guess):
            initial_dir_guess = os.path.dirname(initial_dir_guess)
        ...
        if filepath:
            ...
            if is_video:
                ...
                messagebox.showwarning(
                    "Unknown File Type",
                    f"Could not determine if '{os.path.basename(filepath)}' is a video or image. Assuming video.",
                )
    ...
    def create_widgets(self):
        ...
        self.browse_input_folder_btn = ttk.Button(
            browse_buttons_frame, text="Browse Folder", command=self.browse_input_folder
        )
        ...
        self.browse_single_file_btn = ttk.Button(
            browse_buttons_frame, text="Load Single File", command=self.browse_single_input_file
        )
        ...
        self.widgets_to_disable_during_processing.extend(
            [self.entry_input_dir_or_file, self.browse_input_folder_btn, self.browse_single_file_btn]
        )
        ...
        self.widgets_to_disable_during_processing.append(entry_guidance_scale)
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.append(entry_inference_steps)
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.append(entry_target_width)
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.append(entry_target_height)
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.append(entry_seed)
        row_idx += 1

        # CPU Offload Mode
        ttk.Label(main_params_frame, text="CPU Offload Mode:").grid(row=row_idx, column=0, sticky="e", padx=5, pady=2)
        self.combo_cpu_offload = ttk.Combobox(
            main_params_frame,
            textvariable=self.cpu_offload,
            values=["model", "sequential", "none"],
            width=17,
            state="readonly",
        )
        ...
        self.widgets_to_disable_during_processing.append(self.combo_cpu_offload)
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.append(entry_window_size)
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.append(entry_overlap)
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.append(entry_target_fps)
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.append(entry_process_length)
        row_idx += 1

        # Save Sidecar JSON for Final Output
        self.save_final_json_cb = ttk.Checkbutton(
            fs_frame, text="Save Sidecar JSON for Final Output", variable=self.save_final_output_json_var
        )
        ...
        self.widgets_to_disable_during_processing.append(self.save_final_json_cb)
        row_idx += 1

        # Process as Segments
        self.process_as_segments_cb = ttk.Checkbutton(
            fs_frame,
            text="Process as Segments (Low VRAM Mode)",
            variable=self.process_as_segments_var,
            command=self.toggle_merge_related_options_active_state,
        )
        ...
        self.widgets_to_disable_during_processing.append(self.process_as_segments_cb)
        row_idx += 1

        # --- Merged Output Options Frame ---
        merge_opts_frame = ttk.LabelFrame(
            settings_container_frame, text="Merged Output Options (if segments processed)"
        )
        ...
        self.keep_npz_cb = ttk.Checkbutton(
            merge_opts_frame,
            text="Keep intermediate NPZ",
            variable=self.keep_intermediate_npz_var,
            command=self.toggle_keep_npz_dependent_options_state,
        )
        ...
        self.widgets_to_disable_during_processing.append(self.keep_npz_cb)
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.extend([self.lbl_min_frames_npz, self.entry_min_frames_npz])
        row_idx += 1
        ...
        if OPENEXR_AVAILABLE_GUI:
            combo_intermediate_fmt_values.extend(["exr_sequence", "exr"])
        self.combo_intermediate_fmt = ttk.Combobox(
            merge_opts_frame,
            textvariable=self.keep_intermediate_segment_visual_format_var,
            values=combo_intermediate_fmt_values,
            width=17,
            state="readonly",
        )
        ...
        self.widgets_to_disable_during_processing.extend([self.lbl_intermediate_fmt, self.combo_intermediate_fmt])
        row_idx += 1
        self.toggle_keep_npz_dependent_options_state()

        # Dithering (MP4)
        self.merge_dither_cb = ttk.Checkbutton(
            merge_opts_frame,
            text="Dithering",
            variable=self.merge_dither_var,
            command=self.toggle_dither_options_active_state,
        )
        ...
        self.widgets_to_disable_during_processing.extend(
            [self.merge_dither_cb, self.lbl_dither_str, self.entry_dither_str]
        )
        row_idx += 1
        self.toggle_dither_options_active_state()  # Call after creation

        # Gamma Correct (MP4)
        self.merge_gamma_cb = ttk.Checkbutton(
            merge_opts_frame,
            text="Gamma Adjust",
            variable=self.merge_gamma_correct_var,
            command=self.toggle_gamma_options_active_state,
        )
        ...
        self.widgets_to_disable_during_processing.extend(
            [self.merge_gamma_cb, self.lbl_gamma_val, self.entry_gamma_val]
        )
        row_idx += 1
        self.toggle_gamma_options_active_state()  # Call after creation

        # Percentile Normalization
        self.merge_perc_norm_cb = ttk.Checkbutton(
            merge_opts_frame,
            text="Normalization",
            variable=self.merge_percentile_norm_var,
            command=self.toggle_percentile_norm_options_active_state,
        )
        ...
        ttk.Label(merge_opts_frame, text="  ↳").grid(
            row=row_idx, column=0, sticky="e", padx=(10, 2)
        )  # Aligns with the checkbox
        ...
        self.widgets_to_disable_during_processing.extend(
            [self.lbl_low_perc, self.entry_low_perc, self.lbl_high_perc, self.entry_high_perc]
        )
        row_idx += 1
        ...
        self.combo_merge_alignment = ttk.Combobox(
            merge_opts_frame,
            textvariable=self.merge_alignment_method_var,
            values=["Shift & Scale", "Linear Blend"],
            width=17,
            state="readonly",
        )
        ...
        self.widgets_to_disable_during_processing.extend([lbl_merge_alignment, self.combo_merge_alignment])
        row_idx += 1
        ...
        merge_fmt_values = ["mp4", "main10_mp4", "png_sequence"]
        if OPENEXR_AVAILABLE_GUI:
            merge_fmt_values.extend(["exr_sequence", "exr"])
        self.combo_merge_fmt = ttk.Combobox(
            merge_opts_frame,
            textvariable=self.merge_output_format_var,
            values=merge_fmt_values,
            width=17,
            state="readonly",
        )
        ...
        self.widgets_to_disable_during_processing.extend([lbl_merge_fmt, self.combo_merge_fmt])
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.extend([lbl_merge_suffix, self.entry_merge_suffix])
        row_idx += 1
        ...
        self.enable_secondary_output_cb = ttk.Checkbutton(
            secondary_output_frame,
            text="Enable Secondary Output",
            variable=self.enable_dual_output_robust_norm,
            command=self.toggle_secondary_output_options_active_state,
        )
        ...
        self.widgets_to_disable_during_processing.append(self.enable_secondary_output_cb)
        row_idx += 1

        # Depth Range (0-1) Low / High
        ttk.Label(secondary_output_frame, text="Depth Output Range (0-1):").grid(
            row=row_idx, column=0, sticky="e", padx=5, pady=2
        )
        ...
        self.widgets_to_disable_during_processing.extend([lbl_out_min, entry_out_min, lbl_out_max, entry_out_max])
        row_idx += 1

        # Normalize % Low / High
        ttk.Label(secondary_output_frame, text="Clipped Output % Range:").grid(
            row=row_idx, column=0, sticky="e", padx=5, pady=2
        )
        ...
        self.widgets_to_disable_during_processing.extend([lbl_norm_low, entry_norm_low, lbl_norm_high, entry_norm_high])
        row_idx += 1
        ...
        self.widgets_to_disable_during_processing.extend([lbl_robust_suffix, entry_robust_suffix])
        row_idx += 1
        ...
        start_frame = ttk.Frame(button_container_frame)
        start_frame.pack(side=tk.LEFT, padx=(0, 2))
        ...
        cancel_frame = ttk.Frame(button_container_frame)
        cancel_frame.pack(side=tk.LEFT, padx=(2, 2))
        self.cancel_button = ttk.Button(
            cancel_frame, text="Cancel", command=self.stop_processing, width=10, state=tk.DISABLED
        )
        self.cancel_button.pack(side=tk.LEFT)
        _create_hover_tooltip(self.cancel_button, "cancel_button")

        remerge_frame = ttk.Frame(button_container_frame)
        remerge_frame.pack(side=tk.LEFT, padx=(2, 2))
        self.remerge_button = ttk.Button(
            remerge_frame, text="Re-Merge Segments", command=self.re_merge_from_gui, width=18
        )
        self.remerge_button.pack(side=tk.LEFT)
        _create_hover_tooltip(self.remerge_button, "remerge_button")

        genvis_frame = ttk.Frame(button_container_frame)
        genvis_frame.pack(side=tk.LEFT, padx=(2, 2))
        self.generate_visuals_button = ttk.Button(
            genvis_frame, text="Generate Seg Visuals", command=self.generate_segment_visuals_from_gui, width=20
        )
        self.generate_visuals_button.pack(side=tk.LEFT)
        _create_hover_tooltip(self.generate_visuals_button, "generate_visuals_button")

        self.widgets_to_disable_during_processing.extend(
            [self.start_button, self.remerge_button, self.generate_visuals_button]
        )
    ...
    def generate_segment_visuals_from_gui(self):
        if self.processing_thread and self.processing_thread.is_alive():
            messagebox.showwarning("Busy", "Another process is running. Please wait.")
            return
        meta_file = filedialog.askopenfilename(
            title="Select Master Metadata JSON for Segment Visual Generation",
        ...
        if vis_fmt == "none":
            messagebox.showinfo("Info", "Segment Visual Format is 'none'. Select a valid format.")
            return
        if not messagebox.askyesno(
            "Generate/Overwrite Visuals?",
            f"Generate '{vis_fmt}' visuals for segments in '{os.path.basename(meta_file)}'?\nThis may overwrite existing visuals.",
        ):
        ...
        _logger.info(
            f"--- Starting Segment Visual Generation for: {os.path.basename(meta_file)} (Format: {vis_fmt}) ---"
        )
        self._set_ui_processing_state(True)
        self.processing_thread = threading.Thread(
            target=self._execute_generate_segment_visuals_wrapper, args=(args,), daemon=True
        )
        ...
        if os.path.exists(self.CONFIG_FILENAME):
            try:
                ...
                for key, value in loaded_settings_for_tkvars.items():
                    if key in self.all_tk_vars:
                        try:
                            self.all_tk_vars[key].set(value)
                        except tk.TclError:
                            _logger.warning(
                                f"Warning (GUI load_config): Could not set var {key} during early config load."
                            )
    ...
    def re_merge_from_gui(self):
        if not merge_depth_segments:
            messagebox.showerror("Error", "Merge module not available.")
            return
        meta_file = filedialog.askopenfilename(
            title="Select Master Metadata JSON for Re-Merging",
            filetypes=[("JSON files", "*.json"), ("All files", "*.*")],
            initialdir=self.output_dir.get(),
        )
        if not meta_file:
        ...
        _logger.debug(
            f"DEBUG (re_merge_from_gui): enable_dual_output_robust_norm.get() is {self.enable_dual_output_robust_norm.get()}"
        )
        ...
        if "sequence" in out_fmt:
            parent_dir = filedialog.askdirectory(
                title=f"Select Parent Dir for Re-Merged {out_fmt.upper()} Sequence...", initialdir=self.output_dir.get()
            )
            if parent_dir:
                out_path = parent_dir
            # else:
        ...
        args = {
            "master_meta_path": meta_file,
            "output_path_arg": out_path,
            "do_dithering": self.merge_dither_var.get(),
            "dither_strength_factor": self.merge_dither_strength_var.get(),
            "apply_gamma_correction": self.merge_gamma_correct_var.get(),
            "gamma_value": self.merge_gamma_value_var.get(),
        ...
        self._set_ui_processing_state(True)
        self.processing_thread = threading.Thread(target=self._execute_re_merge_wrapper, args=(args,), daemon=True)
        self.processing_thread.start()
        self.root.after(100, self.process_queue)
    ...
    def start_thread(self):
        ...
        if self.single_file_mode_active:
            ...
            sources_to_process_specs.append(
                {"path": input_path_str, "type": self.current_input_mode, "basename": basename}
            )
        else:
            self.effective_move_original_on_completion = self.MOVE_ORIGINAL_TO_FINISHED_FOLDER_ON_COMPLETION
            if self.current_input_mode == "batch_folder":
                    ...
                    for item_name in os.listdir(input_path_str):
                        ...
                        if os.path.isfile(item_full_path):
                            ...
                            if any(ext in vid_ext.replace("*", "") for vid_ext in self.VIDEO_EXTENSIONS):
                                ...
                                sources_to_process_specs.append(
                                    {"path": item_full_path, "type": "video_file", "basename": basename}
                                )
                        elif os.path.isdir(item_full_path):
                            if self._is_image_sequence_folder(item_full_path):
                                basename = item_name
                                sources_to_process_specs.append(
                                    {"path": item_full_path, "type": "image_sequence_folder", "basename": basename}
                                )
                except NotADirectoryError:
                    _logger.error(
                        f"GUI Input: Path '{input_path_str}' is not a directory, but batch processing mode was attempted."
                    )
                    messagebox.showerror(
                        "Error", f"Input path is not a directory for batch processing: {input_path_str}"
                    )
                    return
                except OSError as e:
                ...
                _logger.critical(
                    f"GUI Start Thread: Unexpected mode '{self.current_input_mode}' for path '{input_path_str}' after explicit determination. This indicates a logic error."
                )
                messagebox.showerror(
                    "Internal Error",
                    f"Unexpected input mode '{self.current_input_mode}' for path '{input_path_str}'. Please report this.",
                )
                return

        if not sources_to_process_specs:
            _logger.warning(
                f"GUI: No valid video files or image sequences found in '{input_path_str}' for mode '{self.current_input_mode}'."
            )
            return
        ...
        if effective_seed_for_run < 0:
            ...
            _logger.debug(
                f"GUI: Seed was set to {gui_seed_setting} (negative). Generating a new random seed for this run: {effective_seed_for_run}"
            )
        else:
        ...
        if final_jobs_to_process_sources:
            ...
            self.processing_thread = threading.Thread(
                target=self._start_processing_wrapper,
            ...
            _logger.info(
                "No videos/segments to process after considering existing data and user choices (or all skipped)."
            )
    ...
    def start_processing(self, source_specs_to_process, effective_seed_for_run):
        ...
        try:
            ...
            self.status_message_var.set("Error: Model initialization failed. See console.")
            self.current_filename_var.set("N/A")
            self.current_resolution_var.set("N/A")
            self.current_frames_var.set("N/A")
            return
        ...
        for source_idx, source_spec in enumerate(source_specs_to_process):
            ...
            if current_gui_mode == "single_video_file" or current_gui_mode == "video_file":
                ...
                _logger.error(
                    f"Lazy Job Definition: Unknown source_spec type '{current_gui_mode}' for basename '{original_basename}'. Skipping."
                )
            ...
            try:
                ...
                _logger.error(
                    f"Skipping {original_basename}: File not found or metadata extraction failed. Error: {e_metadata.__class__.__name__}: {e_metadata}"
                )
            ...
            if is_segment_processing:
                if not all_potential_segments_for_video:
                    reason_skip = (
                        "Too short or invalid overlap/settings"
                        if base_job_info_initial.get("original_video_raw_frame_count", 0) > 0
                        else "Source issue or zero frames/duration"
                    )
                    _logger.info(
                        f"Skipping {original_basename}: No segments defined by settings (Reason: {reason_skip})."
                    )
                    total_sources_processed += 1
                    self.message_queue.put(("progress", total_sources_processed))
                    continue
                ...
                jobs_to_process_for_this_source, action_taken = self._get_segments_to_resume_or_overwrite(
                    ...
                    original_basename,
                    segment_subfolder_path,
                    all_potential_segments_for_video,
                    current_video_base_info_ref,
                )
                _logger.debug(
                    f"For source '{original_basename}': Action '{action_taken}', {len(jobs_to_process_for_this_source)} segments will be processed."
                )

                if not jobs_to_process_for_this_source and not current_video_base_info_ref.get(
                    "pre_existing_successful_jobs"
                ):
                    _logger.info(
                        f"Skipping {original_basename}: Job definition/resume resulted in no segments to process."
                    )
                    total_sources_processed += 1
                    self.message_queue.put(("progress", total_sources_processed))
                    continue
                ...
                full_out_check_path = os.path.join(
                    self.output_dir.get(), get_full_video_output_filename(original_basename, "mp4")
                )
                proceed_full = True
                if os.path.exists(full_out_check_path):
                    if not messagebox.askyesno(
                        "Overwrite?",
                        f"An output file for '{original_basename}' might exist (e.g., MP4):\n{full_out_check_path}\n\nOverwrite if it exists?",
                    ):
                        _logger.info(
                            f"Skipping {original_basename} (full video processing, user chose not to overwrite)."
                        )
                        proceed_full = False
            ...
            all_videos_master_metadata[current_video_path] = self._initialize_master_metadata_entry(
                original_basename, base_job_info_initial, total_expected_jobs_overall
            )
            ...
            pre_existing_successful_segment_metadatas = base_job_info_map[current_video_path].get(
                "pre_existing_successful_jobs", []
            )
            if pre_existing_successful_segment_metadatas:
                _logger.debug(
                    f"Loading {len(pre_existing_successful_segment_metadatas)} pre-existing successful segment metadata entries for {original_basename} into current run's master data."
                )
            ...
            for job_idx, job_info_to_run in enumerate(jobs_to_process_for_this_source):
                ...
                is_segment_job = job_info_to_run.get("is_segment", False)
                log_msg_prefix = (
                    f"Segment {job_info_to_run.get('segment_id', -1) + 1}/{job_info_to_run.get('total_segments', 0)} ({job_idx + 1}/{total_jobs_for_source})"
                    if is_segment_job
                    else "Full video (1/1)"
                )
                ...
                self.status_message_var.set(
                    f"Processing {source_idx + 1} of {len(source_specs_to_process)} ({log_msg_prefix})"
                )

                job_successful, current_job_specific_metadata = self._process_single_job(
                    demo, job_info_to_run, master_meta_for_this_vid
                )

                if current_job_specific_metadata is None:
                    _logger.error(
                        f"Error: _process_single_job for {original_basename} returned None metadata. Initializing to empty dict."
                    )
            ...
            total_accounted_for_vid = (
                master_meta_for_this_vid["completed_successful_jobs"]
                + master_meta_for_this_vid["completed_failed_jobs"]
            )
        ...
        if "demo" in locals() and demo is not None:
            try:
                if hasattr(demo, "pipe") and demo.pipe is not None:
                    if hasattr(demo.pipe, "vae") and demo.pipe.vae is not None:
                        del demo.pipe.vae
                    if hasattr(demo.pipe, "unet") and demo.pipe.unet is not None:
                        del demo.pipe.unet
                    del demo.pipe
                del demo
    ...
    def toggle_dither_options_active_state(self, *args):
        if not (hasattr(self, "process_as_segments_var") and hasattr(self, "merge_dither_var")):
            return
    ...
    def toggle_gamma_options_active_state(self, *args):
        if not (hasattr(self, "process_as_segments_var") and hasattr(self, "merge_gamma_correct_var")):
            return
    ...
    def toggle_keep_npz_dependent_options_state(self, *args):
        if not (
            hasattr(self, "process_as_segments_var")
            and hasattr(self, "keep_intermediate_npz_var")
            and hasattr(self, "keep_npz_dependent_widgets")
        ):
            return
    ...
    def toggle_merge_related_options_active_state(self, *args):
        if not hasattr(self, "process_as_segments_var"):
            return
        ...
        if (
            ...
            and self.cancel_button
        ):
            try:
                if self.start_button.cget("state") == tk.DISABLED and self.cancel_button.cget("state") == tk.NORMAL:
                    current_processing_state = tk.DISABLED
                else:
                    current_processing_state = tk.NORMAL
            except tk.TclError:
                pass
        effective_state_for_merge_options = tk.DISABLED
        if current_processing_state == tk.NORMAL and active:
            effective_state_for_merge_options = tk.NORMAL
        if hasattr(self, "merge_related_widgets_references"):
            for widget_tuple_or_item in self.merge_related_widgets_references:
                items_to_configure = (
                    widget_tuple_or_item if isinstance(widget_tuple_or_item, tuple) else (widget_tuple_or_item,)
                )
                for widget_item in items_to_configure:
                    if hasattr(widget_item, "configure"):
                        try:
                            if isinstance(widget_item, ttk.Combobox):
                                widget_item.configure(
                                    state="readonly" if effective_state_for_merge_options == tk.NORMAL else "disabled"
                                )
                            else:
                                widget_item.configure(state=effective_state_for_merge_options)
                        except tk.TclError:
                            pass
        if not active:
            if current_processing_state == tk.NORMAL:
                for var_attr_name in [
                    "keep_intermediate_npz_var",
                    "merge_dither_var",
                    "merge_gamma_correct_var",
                    "merge_percentile_norm_var",
                ]:
                    if hasattr(self, var_attr_name):
                        var_to_set = getattr(self, var_attr_name)
                        if var_to_set:
                            var_to_set.set(False)
        self.toggle_keep_npz_dependent_options_state()
    ...
    def toggle_percentile_norm_options_active_state(self, *args):
        if not (hasattr(self, "process_as_segments_var") and hasattr(self, "merge_percentile_norm_var")):
            return
    ...
    def toggle_secondary_output_options_active_state(self, *args):
        if not hasattr(self, "enable_dual_output_robust_norm") or not hasattr(
            self, "secondary_output_widgets_references"
        ):
            return
        ...
        for widget_item in self.secondary_output_widgets_references:
            if isinstance(widget_item, tuple):  # Handle cases where we might store (label, entry_frame)
                    ...
                    if hasattr(item, "configure"):
                        try:
                            if isinstance(item, ttk.Combobox):
                                item.configure(state="readonly" if active else "disabled")
                            else:
                ...
                try:
                    if isinstance(widget_item, ttk.Combobox):
                        widget_item.configure(state="readonly" if active else "disabled")
                    else:
                        widget_item.configure(state=state)
                except tk.TclError:
                    pass
