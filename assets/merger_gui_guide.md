...
- **Move to Finish:** Moves the current clip's source files (inpainted video, splatted file, original video, and sidecars) to a `finished` subfolder and advances to the next clip in the list. Useful for when you have done a single process and are happy with the result and wish to skip this from the rest of the batch. Works with or without the Resume checkbox enabled.
