...
class InpaintingGUI(ThemedTk):
    def __init__(self):
        ...
        self.original_input_blend_strength_var = tk.StringVar(
            value=str(self.app_config.get("original_input_blend_strength", 0.0))
        )
        ...
        self.hires_blend_folder_var = tk.StringVar(
            value=self.app_config.get("hires_blend_folder", "./output_splatted_hires")
        )
        ...
        self.inpaint_mask_initial_threshold_var = tk.StringVar(
            value=str(
                self.app_config.get(
                    "inpaint_mask_initial_threshold", self.app_config.get("mask_initial_threshold", 0.3)
                )
            )
        )
        ...
        self.inpaint_mask_morph_kernel_size_var = tk.StringVar(
            value=str(
                self.app_config.get(
                    "inpaint_mask_morph_kernel_size", self.app_config.get("mask_morph_kernel_size", 0.0)
                )
            )
        )
        self.inpaint_mask_dilate_kernel_size_var = tk.StringVar(
            value=str(
                self.app_config.get(
                    "inpaint_mask_dilate_kernel_size", self.app_config.get("mask_dilate_kernel_size", 5)
                )
            )
        )
        self.inpaint_mask_blur_kernel_size_var = tk.StringVar(
            value=str(
                self.app_config.get("inpaint_mask_blur_kernel_size", self.app_config.get("mask_blur_kernel_size", 5))
            )
        )
        ...
        self.enable_post_inpainting_blend = tk.BooleanVar(
    ...
    def _apply_color_transfer(self, source_frame: torch.Tensor, target_frame: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            for i in range(3):  # For L, A, B channels
                target_lab_float[:, :, i] = (target_lab_float[:, :, i] - tgt_mean[i]) / tgt_std[i] * src_std[
                    i
                ] + src_mean[i]
    ...
    def _apply_directional_dilation(self, frame_chunk: torch.Tensor, mask_chunk: torch.Tensor) -> torch.Tensor:
        ...
        try:
            ...
            logger.error(
                f"Error during directional dilation for color transfer reference: {e}. Returning original frames.",
                exc_info=True,
            )
            return frame_chunk
    ...
    def _apply_gaussian_blur(self, mask: torch.Tensor, kernel_size: int) -> torch.Tensor:
        ...
        try:
            ...
            kernel_x = self._create_1d_gaussian_kernel(kernel_val, sigma).to(
                mask.device
            )  # Use derived kernel_val and sigma
            kernel_y = self._create_1d_gaussian_kernel(kernel_val, sigma).to(
                mask.device
            )  # Use derived kernel_val and sigma
            ...
            logger.debug(
                f"Applied Gaussian blur with kernel {kernel_val}x{kernel_val} (derived sigma {sigma:.2f})."
            )  # Updated log message
    ...
    def _apply_mask_dilation(self, mask: torch.Tensor, kernel_size: int) -> torch.Tensor:
        ...
        try:
            ...
            logger.error(
                "Invalid input for mask dilation kernel size. Skipping dilation.", exc_info=True
            )  # Updated log message
            return mask
        except Exception as e:
    ...
    def _apply_morphological_closing(self, mask: torch.Tensor, kernel_size: int) -> torch.Tensor:
        ...
        try:
            ...
            dilated_mask = F.max_pool2d(mask, kernel_size=kernel_val, stride=1, padding=padding)
            ...
            eroded_mask = 1.0 - F.max_pool2d(1.0 - dilated_mask, kernel_size=kernel_val, stride=1, padding=padding)

            logger.debug(
                f"Applied morphological closing with kernel ({kernel_val}x{kernel_val})."
            )  # Updated log message
            return eroded_mask
        except ValueError:
            logger.error(
                "Invalid input for morphological closing kernel size. Skipping closing.", exc_info=True
            )  # Updated log message
            return mask
        except Exception as e:
    ...
    def _process_mask_frames(
        ...
        for t in range(mask_raw.shape[0]):
            ...
            if save_debug and debug_prefix:
                self._save_debug_image(
                    frame_np.astype(np.float32) / 255.0, f"{debug_prefix}_01_mask_raw_color", base_video_name, t
                )
        ...
        if save_debug and debug_prefix:
            self._save_debug_image(
                current_processed_mask, f"{debug_prefix}_02_mask_initial_grayscale", base_video_name, 0
            )
        ...
        if pre_threshold != 0.0:
            ...
            if save_debug and debug_prefix:
                self._save_debug_image(
                    current_processed_mask, f"{debug_prefix}_03_mask_pre_binarized", base_video_name, 0
                )
        ...
        if morph_kernel_size != 0:
            ...
            if save_debug and debug_prefix:
                self._save_debug_image(
                    current_processed_mask, f"{debug_prefix}_04_mask_morph_closed", base_video_name, 0
                )
        ...
        if blur_kernel_size != 0:
            ...
            if save_debug and debug_prefix:
                self._save_debug_image(
                    current_processed_mask, f"{debug_prefix}_06_mask_final_blurred", base_video_name, 0
                )
        ...
        if post_threshold != 0.0:
            ...
            if save_debug and debug_prefix:
                self._save_debug_image(
                    current_processed_mask, f"{debug_prefix}_07_mask_post_binarized", base_video_name, 0
                )
    ...
    def _apply_post_inpainting_blend(
        ...
        if (
            inpainted_frames.shape[0] != original_warped_frames.shape[0]
            or inpainted_frames.shape[2] != original_warped_frames.shape[2]
            or inpainted_frames.shape[3] != original_warped_frames.shape[3]
        ):
            logger.error(
                f"Temporal or Spatial shape mismatch for post-inpainting blend: Inpainted {inpainted_frames.shape} vs Original Warped {original_warped_frames.shape}. Skipping blend."
            )
            return inpainted_frames

        if (
            inpainted_frames.shape[0] != mask.shape[0]
            or inpainted_frames.shape[2] != mask.shape[2]
            or inpainted_frames.shape[3] != mask.shape[3]
            or mask.shape[1] != 1
        ):  # Explicitly check mask has 1 channel
            logger.error(
                f"Mask shape mismatch for post-inpainting blend: Inpainted {inpainted_frames.shape} vs Mask {mask.shape} (Mask must be 1-channel). Skipping blend."
            )
            return inpainted_frames
        ...
        try:
            ...
            if mask_cpu.shape[1] != 1:
                logger.warning(
                    f"Mask has {mask_cpu.shape[1]} channels for blending, expecting 1. Using mean for blending if necessary."
                )
            ...
            if debug_count > 0:
                ...
                for t in range(debug_count):
                    ...
                    cv2.imwrite(
                        os.path.join(
                            debug_output_dir, f"{video_basename_for_debug_blend}_frame_{t:04d}_original_warped.png"
                        ),
                        cv2.cvtColor(original_warped_img, cv2.COLOR_RGB2BGR),
                    )
                    cv2.imwrite(
                        os.path.join(debug_output_dir, f"{video_basename_for_debug_blend}_frame_{t:04d}_inpainted.png"),
                        cv2.cvtColor(inpainted_img, cv2.COLOR_RGB2BGR),
                    )
                    cv2.imwrite(
                        os.path.join(debug_output_dir, f"{video_basename_for_debug_blend}_frame_{t:04d}_mask.png"),
                        mask_img,
                    )
                    cv2.imwrite(
                        os.path.join(debug_output_dir, f"{video_basename_for_debug_blend}_frame_{t:04d}_blended.png"),
                        cv2.cvtColor(blended_img, cv2.COLOR_RGB2BGR),
                    )
            ...
            logger.error(
                f"Error during post-inpainting blending: {e}. Returning original inpainted frames.", exc_info=True
            )
            return inpainted_frames
    ...
    def _create_1d_gaussian_kernel(self, kernel_size: int, sigma: float) -> torch.Tensor:
        ...
        if kernel_size <= 0 or sigma <= 0:
            logger.warning(
                f"Invalid kernel_size ({kernel_size}) or sigma ({sigma}) for Gaussian kernel. Returning identity."
            )
    ...
    def _ensure_file_logging(self, level: int):
        ...
        if self._file_log_handler is None:
            ...
            self._file_log_handler.setFormatter(logging.Formatter("%(asctime)s - %(message)s", datefmt="%H:%M:%S"))
    ...
    def _log_resource_snapshot(
        self, stage: str, base_video_name: str, extra: Optional[dict] = None, level: int = logging.INFO
    ):
    ...
    def _cleanup_all_checkpoints(
        self, base_video_name: str, checkpoint_path: Optional[str] = None, keep_prefinalize: bool = False
    ):
    ...
    def _finalize_output_frames(
        ...
        blend_mask_pre_threshold, blend_mask_post_threshold, blend_mask_morph, blend_mask_dilate, blend_mask_blur = (
            self._resolve_mask_processing_params(
        ...
        if hires_data["is_hires_blend_enabled"]:
            ...
            estimated_channels = (
                ...
                + (
                    frames_left_original_cropped.shape[1]
            ...
            try:
                ...
                frames_warped_hires = torch.empty(
                    (num_frames_original, frames_warpped_original_unpadded_normalized.shape[1], hires_H, hires_W),
                ...
                for chunk_index, i in enumerate(range(0, num_frames_original, chunk_size), start=1):
                    ...
                    lowres_mask_chunk_hires = F.interpolate(
                        lowres_mask[start_idx:end_idx], size=(hires_H, hires_W), mode="bilinear", align_corners=False
                    )
                    ...
                    if blend_mask_source == "lowres":
                        ...
                        mask_chunk_hires = hires_mask_chunk_processed.to(
                            device=lowres_mask_chunk_hires.device, dtype=lowres_mask_chunk_hires.dtype
                        )
                    else:  # hybrid
                        ...
                        mask_chunk_hires = torch.where(
                            hires_mask_chunk_processed > 0, hires_mask_chunk_processed, lowres_mask_chunk_hires
                        )
                ...
                self._save_debug_image(
                    frames_warpped_original_unpadded_normalized, "07a_hires_warped_input", base_video_name, 0
                )
                ...
                gc.collect()
                self._log_resource_snapshot(
                    stage="hires_blend_complete", base_video_name=base_video_name, level=logging.INFO
                )
        ...
        if self.enable_color_transfer.get():
            self._log_resource_snapshot(
                stage="color_transfer_start", base_video_name=base_video_name, level=logging.INFO
            )
            ...
            if is_dual_input:
                ...
                logger.debug(
                    "Dual input detected. Creating occlusion-free reference via directional dilation for color transfer..."
                )
                ...
                if self.debug_mode_var.get():
                    ...
                    for t in range(min(5, reference_frames_for_transfer.shape[0])):
                        ref_img = (reference_frames_for_transfer[t].permute(1, 2, 0).cpu().numpy() * 255).astype(
                            np.uint8
                        )
                        cv2.imwrite(
                            os.path.join(
                                debug_output_dir, f"{video_basename_for_debug}_frame_{t:04d}_color_ref_dilated.png"
                            ),
            ...
            if reference_frames_for_transfer is None or reference_frames_for_transfer.numel() == 0:
                ...
                self._log_resource_snapshot(
                    stage="color_transfer_complete", base_video_name=base_video_name, level=logging.INFO
                )
        ...
        if self.enable_post_inpainting_blend.get():
            ...
            self._log_resource_snapshot(
                stage="post_blend_complete", base_video_name=base_video_name, level=logging.INFO
            )
        ...
        if is_dual_input:
            ...
            if frames_left_original_cropped is None or frames_left_original_cropped.numel() == 0:
                logger.error(
                    f"Original left frames are missing or empty for non-dual input {base_video_name}. Cannot create SBS output."
                )
                return None

            # Ensure dimensions match before concatenation
            if (
                frames_left_original_cropped.shape[0] != frames_output_final.shape[0]
                or frames_left_original_cropped.shape[1] != frames_output_final.shape[1]
                or frames_left_original_cropped.shape[2] != frames_output_final.shape[2]
            ):
                logger.error(
                    f"Dimension mismatch for SBS concatenation: Left {frames_left_original_cropped.shape}, Inpainted {frames_output_final.shape} for {base_video_name}."
                )
                return None
    ...
    def _find_high_res_match(self, low_res_video_path: str) -> Optional[str]:
        ...
        if not base_pattern_no_dim:
            logger.warning(
                f"Failed to find true base name for {low_res_filename} after stripping resolution. Skipping Hi-Res match."
            )
            return None
        ...
        if len(matches) > 1:
            logger.warning(
                f"Multiple Hi-Res matches found for {low_res_filename}. Using the first match: {os.path.basename(matches[0])}"
            )
        ...
        if hires_path:
            ...
            if hires_w_raw <= low_res_w_raw:  # Check if Hi-Res is NOT strictly higher resolution
                logger.warning(
                    f"Hi-Res candidate {os.path.basename(hires_path)} ({hires_w_raw}px) is not higher resolution than Low-Res ({low_res_w_raw}px). Disabling Hi-Res blending."
                )
                return None
    ...
    def _prepare_video_inputs(
        ...
        """
        frames, fps, video_stream_info = read_video_frames_torch(input_video_path)
        ...
        if process_length != -1 and process_length > 0:
            ...
            logger.info(
                f"Limiting processing to first {actual_frames_to_process_count} frames (out of {total_frames_in_video})."
            )
            ...
            if update_info_callback:
                self.after(
                    ...
                    lambda: update_info_callback(
                        base_video_name,
                        "N/A",
                        f"0 (out of {total_frames_in_video})",
                        overlap,
        ...
        if num_frames_original == 0:
            ...
            if update_info_callback:
                self.after(
                    0,
                    lambda: update_info_callback(
                        base_video_name, "N/A", "0 (skipped)", overlap, original_input_blend_strength
                    ),
                )
            return None
        ...
        if new_h % required_divisor != 0:
            ...
            logger.warning(
                f"Video height {total_h_raw_input_before_resize} is not divisible by {required_divisor}. Resizing to {new_h}."
            )

        if new_w % required_divisor != 0:
            new_w = (new_w // required_divisor + 1) * required_divisor
            logger.warning(
                f"Video width {total_w_raw_input_before_resize} is not divisible by {required_divisor}. Resizing to {new_w}."
            )
        ...
        if new_h != total_h_raw_input_before_resize or new_w != total_w_raw_input_before_resize:
            if frames.shape[0] > 0:
                ...
                logger.info(
                    f"Frames resized from {total_h_raw_input_before_resize}x{total_w_raw_input_before_resize} to {new_h}x{new_w}."
                )
            else:
        ...
        if total_h_current < required_divisor or total_w_current < required_divisor:
            error_msg = (
                f"Video {base_video_name} is too small after resize ({total_w_current}x{total_h_current}), skipping."
            )
            ...
            if update_info_callback:
                self.after(
                    ...
                    lambda: update_info_callback(
                        base_video_name,
                        f"{total_w_current}x{total_h_current} (INVALID)",
                        num_frames_original,
                        overlap,
                        original_input_blend_strength,
                    ),
        ...
        if is_dual_input:
            ...
            if output_display_h % required_divisor != 0 or output_display_w % required_divisor != 0:
                ...
                if update_info_callback:
                    self.after(
                        0,
                        lambda: update_info_callback(
                            base_video_name, f"{output_display_w}x{output_display_h} (INVALID)", "Skipped", "N/A", "N/A"
                        ),
                    )
            ...
            frames_left_original_cropped = (
                frames_left_original_full_padded[:num_frames_original].float() / 255.0
        ...
        if update_info_callback:
            display_frames_info = (
                f"{actual_frames_to_process_count} (out of {total_frames_in_video})"
                if process_length != -1
                else str(total_frames_in_video)
            )
            self.after(
                0,
                lambda: update_info_callback(
                    base_video_name,
                    f"{output_display_w}x{output_display_h}",
                    display_frames_info,
                    overlap,
                    original_input_blend_strength,
                ),
        ...
        return (
            frames_warpped_padded,
            frames_inpaint_mask_padded,
            frames_left_original_cropped,
            num_frames_original,
            padded_H,
            padded_W,
    ...
    def create_widgets(self):
        ...
        self.help_menu.add_checkbutton(
            label="Enable Debugging", variable=self.debug_mode_var, command=self._toggle_debug_mode
        )
        ...
        ttk.Entry(param_frame, textvariable=self.num_inference_steps_var, width=10).grid(
        ...
        ttk.Entry(param_frame, textvariable=self.original_input_blend_strength_var, width=10).grid(
        ...
        ttk.Entry(param_frame, textvariable=self.tile_num_var, width=10).grid(
            row=current_row, column=1, sticky="w", padx=5
        )
        ...
        Tooltip(
            process_length_label,
            self.help_data.get("process_length", "Number of frames to process. Use -1 for all frames."),
        )
        ttk.Entry(param_frame, textvariable=self.process_length_var, width=10).grid(
            row=current_row, column=1, sticky="w", padx=5
        )
        ...
        ttk.OptionMenu(param_frame, self.offload_type_var, self.offload_type_var.get(), *offload_options).grid(
            row=current_row, column=3, sticky="w", padx=5
        )
        current_row += 1
        ...
        Tooltip(
            single_clip_id_label,
            self.help_data.get("single_clip_id", "Optional clip ID filter. Leave blank to process all clips."),
        )
        ...
        blend_enable_check = ttk.Checkbutton(
            post_process_frame,
            text="Enable Post-Inpainting Blend",
        ...
        color_transfer_check = ttk.Checkbutton(
            post_process_frame,
            text="Enable Color Transfer",
            variable=self.enable_color_transfer,
            command=self._toggle_color_transfer_state,
        )  # Will create this command
        color_transfer_check.grid(
            row=current_row, column=2, columnspan=2, sticky="w", padx=5, pady=2
        )  # Occupies right side
    ...
    def process_single_video(
        ...
        if resume_checkpoint is not None:
            ...
            try:
                ...
                if recomputed_inputs is not None:
                    ...
                    if recomputed_blend_mask.shape == frames_blend_mask_processed_unpadded_original_length.shape:
                        ...
                        logger.warning("Recomputed blend mask shape mismatch; using cached blend mask from checkpoint.")
            ...
            for i in range(0, total_frames_to_process_actual, stride):
                ...
                if padding_needed_for_pipeline_input > 0:
                    logger.debug(
                        f"Dynamically padding input for chunk starting at frame {i}: {actual_sliced_length} frames sliced, {padding_needed_for_pipeline_input} frames needed."
                    )
                    last_original_frame_warpped = (
                        frames_warpped_padded[total_frames_to_process_actual - 1].unsqueeze(0).clone()
                    )
                    last_original_frame_mask = (
                        frames_inpaint_mask_padded[total_frames_to_process_actual - 1].unsqueeze(0).clone()
                    )
                ...
                if previous_chunk_output_frames is not None and overlap > 0:
                    ...
                    if overlap_actual > 0:
                        ...
                        if original_input_blend_strength > 0:
                            ...
                            original_weights_scaled = (
                                torch.linspace(0.0, 1.0, overlap_actual, device=prev_gen_overlap_frames.device).view(
                                    -1, 1, 1, 1
                                )
                                * original_input_blend_strength
                            )
                            blended_input_overlap_frames = (
                                1 - original_weights_scaled
                            ) * prev_gen_overlap_frames + original_weights_scaled * orig_input_overlap_frames
                ...
                # --- INFERENCE ---
                logger.info(
                    f"Starting inference for chunk {i}-{i + input_frames_to_pipeline.shape[0]} (Temporal length: {input_frames_to_pipeline.shape[0]})..."
                )
                start_time = time.time()

                with torch.no_grad():
                    video_latents = spatial_tiled_process(
                        cond_frames=input_frames_to_pipeline,
                ...
                logger.debug(
                    f"Inference for chunk {i}-{i + input_frames_to_pipeline.shape[0]} completed in {inference_duration:.2f} seconds."
                )
                ...
                if i == 0:
                    ...
                    if overlap > 0 and previous_chunk_output_frames is not None:
                        ...
                        weights = torch.linspace(0.0, 1.0, overlap, device=current_chunk_generated.device).view(
                            -1, 1, 1, 1
                        )
                        ...
                        next_i = i + stride
                        is_last = (next_i >= total_frames_to_process_actual) or (
                            total_frames_to_process_actual - next_i <= overlap
                        )
            ...
            if not results:
                ...
                if update_info_callback:
                    self.after(
                        0,
                        lambda: update_info_callback(
                            base_video_name, "N/A", "0 (No Output)", overlap, original_input_blend_strength
                        ),
                    )
                return False, None
            ...
            if frames_output.numel() == 0 or frames_output.shape[2] < padded_H or frames_output.shape[3] < padded_W:
                logger.error(
                    f"Generated frames_output has invalid dimensions (actual {frames_output.shape[2]}x{frames_output.shape[3]} vs target {padded_H}x{padded_W})."
                )
                return False, None
            ...
            self._log_resource_snapshot(
                ...
                base_video_name=base_video_name,
                extra={
                    "num_frames": int(frames_output_final.shape[0]),
                    "resolution": f"{int(padded_W)}x{int(padded_H)}",
                },
                level=logging.INFO,
            )
        ...
        if finalized_outputs is None:
            ...
            if update_info_callback:
                self.after(
                    0,
                    lambda: update_info_callback(
                        base_video_name, "N/A", "0 (Empty Final)", overlap, original_input_blend_strength
                    ),
                )
            return False, None
        ...
        try:
            ...
            encoding_success = encode_frames_to_mp4(
                ...
                video_stream_info=video_stream_info,
                stop_event=stop_event_non_optional,
                sidecar_json_data=None,
                ...
                if not filtered_videos:
                    self.after(
                        ...
                        lambda: messagebox.showinfo(
                            "Info", f"No .mp4 files matched Single Clip ID {single_clip_id} in input folder."
                        ),
                    )
                    self.after(0, self.processing_done)
                    return
                    ...
                    try:
                        ...
                        if "frame_overlap" in sidecar_data:
                            ...
                            if sidecar_overlap >= 0:
                                ...
                                logger.warning(
                                    f"Invalid 'frame_overlap' in sidecar file for {os.path.basename(video_path)}. Using GUI value ({gui_overlap})."
                                )
                        ...
                        if "input_bias" in sidecar_data:
                            ...
                            if 0.0 <= sidecar_input_bias <= 1.0:
                                ...
                                logger.warning(
                                    f"Invalid 'input_bias' in sidecar file for {os.path.basename(video_path)}. Using GUI value ({gui_original_input_blend_strength})."
                                )
                        ...
                        if "output_crf" in sidecar_data:
                            ...
                            if sidecar_crf >= 0:
                                ...
                                logger.warning(
                                    f"Invalid 'output_crf' in sidecar file for {os.path.basename(video_path)}. Using GUI value ({gui_output_crf})."
                                )
                        ...
                        if "process_length" in sidecar_data:
                            ...
                            if sidecar_process_length == -1 or sidecar_process_length > 0:
                                ...
                                logger.warning(
                                    f"Invalid 'process_length' in sidecar file for {os.path.basename(video_path)}. Using GUI value ({process_length})."
                                )

                    except (json.JSONDecodeError, ValueError) as e:
                        logger.warning(
                            f"Error reading or parsing sidecar file {json_path}: {e}. Falling back to GUI parameters for this video."
                        )
                else:
    ...
    def start_processing(self):
        ...
        try:
            ...
            if (
                ...
                or frames_chunk < 1
                or gui_overlap < 0
                or not (0.0 <= gui_original_input_blend_strength <= 1.0)
                or gui_output_crf < 0
            ):  # NEW VALIDATION for CRF
                raise ValueError("Invalid parameter values")
        except ValueError:
            # UPDATED ERROR MESSAGE
            messagebox.showerror(
                "Error",
                "Please enter valid values: Inference Steps >=1, Tile Number >=1, Frames Chunk >=1, Frame Overlap >=0, Original Input Bias between 0.0 and 1.0, Output CRF >=0, Process Length is -1 or >0, and Single Clip ID is blank or >=0.",
            )
            return
        offload_type = self.offload_type_var.get()
    ...
    def load_settings(self):
        ...
        filename = filedialog.askopenfilename(
            defaultextension=".json", filetypes=[("JSON files", "*.json")], title="Load Settings from File"
        )
        if not filename:
            return
    ...
    def save_settings(self):
        ...
        filename = filedialog.asksaveasfilename(
            defaultextension=".json", filetypes=[("JSON files", "*.json")], title="Save Settings to File"
        )
        if not filename:
            return
...
def read_video_frames_torch(video_path: str, decord_ctx=cpu(0)) -> Tuple[torch.Tensor, float, Optional[dict]]:
    """
    ...
    frames_numpy, fps, _, _, _, _, video_stream_info = read_video_frames_decord(
        video_path=video_path, decord_ctx=decord_ctx
    )
...
def spatial_tiled_process(
    ...
    **kwargs,
    ...
    if not final_rows:
        logger.error(
            "No final rows to concatenate after spatial tiling. This indicates a major issue with tile processing or blending."
        )
