...
try:
    ...
    _logger.warning(
        "OpenEXR/Imath libraries not found. EXR features will be limited/unavailable. Context: merge_depth_segments.py"
    )
...
def _apply_robust_global_normalization(
    ...
    if effective_raw_closest_val <= effective_raw_farthest_val + 1e-6:  # Add small epsilon to handle near-zero range
        _logger.warning(
            f"Robust normalization: Effective closest raw val ({effective_raw_closest_val:.4f}) is effectively <= effective farthest raw val ({effective_raw_farthest_val:.4f}). Returning uniform output."
        )
    ...
    _logger.debug(
        f"Robust norm: Effective raw farthest={effective_raw_farthest_val:.4f}, closest={effective_raw_closest_val:.4f}"
    )
...
def _load_and_validate_metadata(master_meta_path: str):
    ...
    if N_overlap_from_meta is None:
        _logger.critical(
            f"'segment_definition_output_overlap_frames' not found in metadata: {master_meta_path}. Aborting merge."
        )
    ...
    successful_jobs_info = [
        job for job in jobs_info if job.get("status") == "success" and job.get("output_segment_filename")
    ]
...
def _load_multiple_segments_data(sorted_jobs_info: list, base_dir: str):
    ...
    for idx, job_meta in enumerate(sorted_jobs_info):
        ...
        if determined_fps is None:
            ...
            _logger.warning(
                f"Warning: Inconsistent FPS. Using {determined_fps:.2f}. Segment {segment_id} has {current_fps:.2f}."
            )

        _logger.debug(f"Loading segment {segment_id} ({idx + 1}/{len(sorted_jobs_info)}): {segment_filename}")
        if input_segment_format != "npz":
            _logger.critical(
                f"Unsupported segment format '{input_segment_format}' for {segment_filename}. Expecting NPZ."
            )
...
def _align_segments_data(
    all_loaded_segments_frames: list, segment_job_meta_map: list, N_overlap: int, merge_alignment_method: str
):
    if not all_loaded_segments_frames:
        return []
    if len(all_loaded_segments_frames) == 1:
        return all_loaded_segments_frames
    ...
    for idx in range(1, len(all_loaded_segments_frames)):
        ...
        _logger.debug(
            f"Aligning segment (ID {current_id}) to previous (ID {prev_id}). Method: {merge_alignment_method}"
        )
            ...
            if eff_overlap > 0:
                if merge_alignment_method.lower() == "shift_scale":
                    ...
                    s, t = util.compute_scale_and_shift_full(
                        pred_align_frames.reshape(-1), target_align_frames.reshape(-1), mask
                    )
                    _logger.debug(
                        f"Aligning segment {current_id} to {prev_id}. Method: shift_scale. Scale: {s:.4f}, Shift: {t:.4f}"
                    )
                    aligned_current = s * current_raw + t
                elif merge_alignment_method.lower() == "linear_blend":
                    _logger.debug(
                        f"  Linear Blend: No explicit S&S alignment for segment ID {current_id}. Blending will occur in stitching."
                    )
                else:
                    _logger.warning(
                        f"CRITICAL WARNING: Unknown alignment method '{merge_alignment_method}'. No alignment performed on segment ID {current_id}."
                    )
            else:
                _logger.debug(
                    f"Warning: No actual overlap for alignment between {prev_id} and {current_id}. No S&S alignment performed."
                )
        else:
    ...
    if N_overlap == 0:
        ...
        for i, segment_frames in enumerate(all_aligned_segments):
            ...
            if len(segment_frames) > 0:
                final_frames_list.extend(list(segment_frames))
    else:
        for idx, current_segment_aligned in enumerate(all_aligned_segments):
            ...
            if idx == 0:
                frames_to_add_count = (
                    len(current_segment_aligned) - N_overlap
                    if len(all_aligned_segments) > 1
                    else len(current_segment_aligned)
                )
                ...
                if eff_blend_len <= 0:
                    _logger.warning(
                        f"  Warning: No frames for blending between {prev_seg_id} and {seg_id}. Hard cut implies adding all of current."
                    )
                    if len(current_segment_aligned) > 0:
                    ...
                    blended_frames = (
                        util.get_interpolate_frames(blend_pre_frames, blend_post_frames)
                        if eff_blend_len > 1
                        else [(0.5 * blend_pre_frames[0] + 0.5 * blend_post_frames[0])]
                        if eff_blend_len == 1
                        else []
                    )
                    ...
                    if idx == len(all_aligned_segments) - 1:
                        frames_after_blend = (
                            current_segment_aligned[start_idx_for_remainder:]
                            if start_idx_for_remainder < len(current_segment_aligned)
                            else []
                        )
                        ...
                        frames_after_blend = (
                            current_segment_aligned[start_idx_for_remainder:end_idx_for_remainder]
                            if start_idx_for_remainder < end_idx_for_remainder
                            else []
                        )
...
def _apply_mp4_postprocessing_refactored(
    video_normalized: np.ndarray, apply_gamma: bool, gamma_val: float, do_dithering: bool, dither_strength: float
):
...
def _determine_output_path(
    ...
    out_format: str,
    final_filename_base_override: Optional[
        str
    ] = None,  # This is the complete desired base filename (e.g., "myvideo_depth" or "myvideo_clipped_depth")
) -> str:
    """
    ...
    effective_base_filename = (
        final_filename_base_override if final_filename_base_override else original_basename_from_meta
    )
    ...
    if out_path_arg:
        if os.path.isdir(out_path_arg):  # If out_path_arg is an existing directory
        ...
        elif not os.path.exists(out_path_arg) and "." not in os.path.basename(
            out_path_arg
        ):  # If it's a non-existing path without extension, assume directory
            target_output_dir = out_path_arg
            ...
            if not target_output_dir:
                target_output_dir = "."  # Fallback if only a filename was provided
...
def merge_depth_segments(
    ...
    _logger.info(
        f"Starting depth segment merging process... Format: {output_format}, Alignment: {merge_alignment_method}"
    )
    ...
    _logger.debug(
        f"Merge Settings - Standard Percentile Norm: {use_percentile_norm}, Low: {norm_low_percentile}%, High: {norm_high_percentile}%"
    )
    if enable_dual_output_robust_norm:
        _logger.debug(
            f"Merge Settings - Robust Dual Output Enabled. Low: {robust_norm_low_percentile}%, High: {robust_norm_high_percentile}%, Out Min: {robust_norm_output_min}, Out Max: {robust_norm_output_max}, Suffix: '{robust_output_suffix}'"
        )
    ...
    try:
        ...
        if len(sorted_jobs) == 1:
            ...
            if len(loaded_frames_list) == 1:
                ...
                aligned_segments = _align_segments_data(
                    loaded_frames_list, job_meta_map, N_overlap, merge_alignment_method
                )
        ...
        if "mp4" in output_format.lower():  # Covers "mp4", "main10_mp4"
            standard_video_to_save = _apply_mp4_postprocessing_refactored(
                standard_normalized_video, apply_gamma_correction, gamma_value, do_dithering, dither_strength_factor
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Override the base for the output filename (e.g., 'myvideo_remerged').",
    )
    parser.add_argument(
        "--output_path", "-o", type=str, default=None, help="Output path for video or sequence directory."
    )
    ...
    parser.add_argument(
        "--output_format", type=str, default="mp4", choices=output_format_choices, help="Output format."
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="shift_scale",
        choices=["shift_scale", "linear_blend"],
        help="Segment alignment method.",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="info",
        choices=list(verbosity_choices.keys()),
    ...
    logging.basicConfig(
        level=verbosity_choices[args.verbosity],
