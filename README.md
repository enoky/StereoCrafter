...
- **torch.compile (V1)**: the V1 Inpainting GUI can compile the UNet and VAE decoder ("Use torch.compile" checkbox) — measured 1.09× UNet / 1.32× VAE decode (~15% per chunk), exact math, stacking with SageAttention. Warmup runs during the first chunk and looks stalled (it isn't): ~9 min the first time ever, ~3 min in later sessions via `torchinductor_cache/`. Net win for movie-scale batches (break-even ≈ 4 min of footage); leave it off for single short clips.
