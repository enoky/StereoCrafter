...
class FusionSidecarGenerator:
    ...
    def _update_status(self, text):
        """Safely updates the status label on the master GUI."""
        self.master_gui.after(0, lambda: self.master_gui.status_label.config(text=text))
    ...
    def generate_sidecars(self, export_file_path, target_folder):
        ...
        if applied_count == 0:
            self._update_status(
                "Finished: No parameters were applied from the export file."
            )
        else:
            self._update_status(
                f"Finished: Applied markers to {applied_count} files, generated {len(target_videos)} FSSIDECARs."
            )

        self.master_gui.after(
            0,
    ...
    def generate_custom_sidecars(self, export_file_path, dest_folder, custom_base):
        """Generates sidecars with a custom base name, one for each marker in the export file."""

        if not export_file_path or not dest_folder or not custom_base:
            return
        ...
        for i, marker in enumerate(markers):
            ...
            # Use marker index and frame number in filename for clarity
            json_filename = os.path.join(
                dest_folder, f"{custom_base}_{i + 1:03d}.fssidecar"
            )

            if not self.sidecar_manager.save_sidecar_data(json_filename, sidecar_data):
                self.logger.error(f"Failed to save sidecar for marker {i + 1}.")

            last_param_vals = current_param_vals.copy()

        # 5. Final Status
        self._update_status(f"Finished: Generated {len(markers)} custom sidecar files.")
        self.master_gui.after(
            0,
            lambda: messagebox.showinfo(
                "Sidecar Generation Complete",
                f"Successfully generated {len(markers)} FSSIDECAR files in: {dest_folder}",
            ),
        )
...
class SplatterGUI(ThemedTk):
    ...
    def run_fusion_sidecar_generator(self):
        ...
        def worker():
            ...
            generator.generate_sidecars(export_file_path, target_folder)

        threading.Thread(target=worker, daemon=True).start()

    def run_fusion_sidecar_generator_custom(self):
        """Initializes and runs the FusionSidecarGenerator tool in custom mode."""
        export_file_path = filedialog.askopenfilename(
            defaultextension=".fsexport",
            filetypes=[
                ("Fusion Export Files", "*.fsexport.txt;*.fsexport"),
        ...
        default_base = os.path.splitext(os.path.basename(export_file_path))[0]
        if default_base.endswith(".fsexport"):
            default_base = os.path.splitext(default_base)[0]

        save_path = filedialog.asksaveasfilename(
            title="Set Sidecar Base Name and Destination",
            initialfile=default_base,
            defaultextension=".fssidecar",
            filetypes=[("Sidecar Files", "*.fssidecar"), ("All Files", "*.*")],
        )
        if not save_path:
            return

        dest_folder = os.path.dirname(save_path)
        custom_base = os.path.splitext(os.path.basename(save_path))[0]
        ...
        def worker():
            ...
            generator.generate_custom_sidecars(
                export_file_path, dest_folder, custom_base
            )
